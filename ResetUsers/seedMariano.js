...
module.exports = async function seedMariano () {
  ...
  const { rows } = await pool.query(
    `INSERT INTO users (email, username, password, fullname, registrationday, verified)
     ...
     RETURNING id`,
    [EMAIL, 'mariano', passHash, 'Mariano', TODAY()]
  );
  ...
  const actValues = actIds.map((id, i) =>
    `(${userId}, ${id}, ${25 + i * 15}, NULL, NULL, '${daysAgoUTC(i)}')`
  ).join(',');
  await pool.query(
